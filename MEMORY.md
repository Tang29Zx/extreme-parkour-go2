...
- 本地单环境视觉回放支持显式传入 `--cpu_camera`：相机仍由NVIDIA/Vulkan
  ...
  GPU tensor相机路径。
- 随机箱视觉任务的深度有效距离为`3.0 m`；`2.0～3.0 m`额外噪声随距离
  线性增强，最远处增加`0.02 m`高斯标准差和3%无效点概率。深度GRU会按
  episode的`dones`逐环境清空，避免跨布局残留；`delta_yaw_ok`使用
  `abs(delta_yaw) < 0.6`，其余yaw蒸馏语义暂不改变。
