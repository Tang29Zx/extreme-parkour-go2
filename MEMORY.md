...
- 2026-07-29 `go2_random_box`训练相机已按后续决定恢复到该任务创建时的原始
  光心分布：均值`(0.355, 0, 0.065) m`、标准差
  `(0.015, 0.010, 0.015) m`；pitch随机范围改为向下`22.5～27.5°`，中心
  `25°`，roll/yaw仍为`±2°`。此前`(0.344, 0.0175, 0.080) m`仅为未实测的
  工程估算，不再作为当前训练配置。
- 2026-07-29 `go2_random_box`训练光心随后按用户要求沿base `+Z`从原始
  `0.065 m`上移`0.020 m`，当前均值为`(0.355, 0, 0.085) m`；位置标准差
  仍为`(0.015, 0.010, 0.015) m`，pitch仍为向下`22.5～27.5°`，roll/yaw
  仍为`±2°`。基础`go2`配置继续保持`(0.355, 0, 0.065) m`。
...
- Onboard将calf稳态步长从0.40降到0.32、hip/thigh保持0.30后，用与0.40档逐环境
  相同的seed 17/18完整随机化样本复测0.25 m共40局：成功由12/40降至8/40，安全
  硬停由7增至13（9次速度越界、4次calf联合可行区间为空），最大速度比例由1.08637
  升至1.43238。目标步长/持续约束/任意裁剪率由6.83%/12.21%/22.67%升至
  9.85%/15.04%/26.66%，固定降低calf步长没有改善本批安全或成功率。结果位于
  `artifacts/onboard_model_38300_step030_calf032_box025_trials20_seed{17,18}.json`；
  URDF速度/扭矩、权重、随机化和其他部署边界均未改变。该0.32实验随后已按用户要求
  回滚，Onboard当前calf稳态步长恢复为0.40；实验JSON继续保留作为反例证据。
